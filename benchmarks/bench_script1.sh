...
for table in ${memtable[@]}; do
    for data in ${data_size[@]}; do
        for sf in ${scale_factor[@]}; do
            ...
            echo "record_count tree_height sample_size insert_time rejections attempts cache_misses buffer_proc_time bounds_time walker_time sample_time rej_check_time" > $fname

            echo "Running trials for $data $table $sf"
            for i in $(seq 1 $trials); do
                benchmarks/first_bench "$data" "$table" "$sf" 0 0 >> "$fname"
