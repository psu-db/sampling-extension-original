...
static void load_data(std::fstream *file, lsm::sampling::LSMTree *lsmtree, size_t count) 
{
    std::string line;
    size_t i=0;
    while (i < count && getline(*file, line, '\n')) {
        ...
        std::byte *key = (std::byte *) key_field.c_str();
        std::byte *val = (std::byte *) value_field.c_str();

        lsmtree->insert(key, val);
        i++;
    }
}
...
static bool benchmark(lsm::sampling::LSMTree *tree, std::fstream *file, 
    ...
    fprintf(stdout, "%ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld %ld\n", tree->record_count(), tree->depth(), sample_size, per_insert, rejections / samples, attempts / samples, cache_misses / samples, buffer_time / samples, bounds_time / samples, walker_time / samples, sample_time / samples, io_time / samples, reject_time / samples);
...
int main(int argc, char **argv)
{
    ...
    // records
    size_t initial_insertions = 1000000000;
    load_data(&datafile, sampling_lsm.get(), initial_insertions);

    size_t sample_size = 1000;
    size_t samples = 10000;
    size_t inserts = 500000000;
