...
START_TEST(t_initialize)
{
    ...
    ck_assert_int_eq(io::PagedFile::initialize(dfile1.get(), test_flid1), 1);
    
    // initializing a closed dfile should fail
    ck_assert_int_eq(io::PagedFile::initialize(dfile2.get(), test_flid2), 0);
    ...
    io::PagedFileHeaderData *header = (io::PagedFileHeaderData *) buf;
    ...
    ck_assert_int_eq(header->page_count, 0);
    ck_assert_int_eq(header->flid, test_flid1);
}
END_TEST
...
START_TEST(t_constructor_empty)
{
    ...
    auto pfile = io::PagedFile(std::move(dfile), false);
...
START_TEST(t_write)
{
    ...
    int fd = dfile->get_fd();
    pfile = new io::PagedFile(std::move(dfile), false);
...
START_TEST(t_read)
{
    ...
    io::PagedFile pfile = io::PagedFile(std::move(dfile), false);
...
Suite *unit_testing()
{
    ...
    TCase *initialize = tcase_create("lsm::io::PagedFile::initialize Testing");
    tcase_add_test(initialize, t_initialize);

    suite_add_tcase(unit, initialize);


    TCase *constructor = tcase_create("lsm::io::PagedFile::PagedFile() Testing");
    tcase_add_test(constructor, t_constructor_empty);

    suite_add_tcase(unit, constructor);


    TCase *allocate = tcase_create("lsm::io::PagedFile::allocate Testing");
    ...
    TCase *free_page = tcase_create("lsm::io::PagedFile::free_page Testing");
    tcase_add_test(free_page, t_free_page);

    suite_add_tcase(unit, free_page);


    TCase *write = tcase_create("lsm::io::PagedFile::write Testing");
    tcase_add_test(write, t_write);

    suite_add_tcase(unit, write);


    TCase *read = tcase_create("lsm::io::PagedFile::read Testing");
