...
struct btree_default_traits {
    ...
    static const int inner_slots = 4;
...
private:
    ...
    struct InnerNode : public node {
        ...
        uint64_t weight[inner_slotmax + 1];
    ...
    static uint64_t calculate_weight(const node* n) {
        if (!n) return 0;
        else if (n->is_leafnode()) return n->slotuse;
        uint64_t res = 0;
...
public:
    ...
    void range_sample(const key_type& lower, const key_type& upper, size_t k, std::vector<key_type>& ans, gsl_rng *rng) {
        ...
        const node* lca = n;
        uint64_t lca_size_cnt = calculate_weight(lca);
        ans.clear();
        while (ans.size() < k) {
            const node* now = lca;
            ...
            unsigned short p = gsl_rng_uniform(rng) * now->slotuse;
            key_type sample = static_cast<const LeafNode*>(now)->key(p);
...
private:
    ...
    result_t erase_one_descend(const key_type& key,
        ...
        else // !curr->is_leafnode()
        {
            ...
            } else if (!result.has(btree_shift)) --inner->weight[slot];
