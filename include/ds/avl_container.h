...
template<class Value, class RankType, bool StoreWeight, class = void>
struct Node {
    Value           m_value;
...
template<class Value, class RankType, class KeyFunc, bool UnifWeight>
struct NodeDescBase0:
    public avl_impl::AVLNodeDescBase<
        Node<Value, RankType, std::is_arithmetic_v<RankType> && !UnifWeight>,
        KeyType<Value, KeyFunc>> {

    constexpr const static bool m_store_weight =
        std::is_arithmetic_v<RankType> && !UnifWeight;
    typedef Node<Value, RankType, m_store_weight> _node_t;
};

template<class Value, class RankType, class KeyFunc, bool UnifWeight>
struct NodeDescBase1: public NodeDescBase0<Value, RankType, KeyFunc, UnifWeight> {
    using Base = NodeDescBase0<Value, RankType, KeyFunc,  UnifWeight>;
    using Base::m_store_weight;
    using typename Base::_node_t;

    NodeDescBase1(const KeyFunc &key_func): m_key_func(key_func) {}
    ...
    void swap(NodeDescBase1 &other) noexcept {
        swap(m_key_func, other.m_key_func);
    }
};

template<class Value, class RankType, bool UnifWeight>
struct NodeDescBase1<Value, RankType, ident_key<Value>, UnifWeight>:
    public NodeDescBase0<Value, RankType, ident_key<Value>, UnifWeight> {
    using Base = NodeDescBase0<Value, RankType, ident_key<Value>, UnifWeight>;
    using Base::m_store_weight;
    using typename Base::_node_t;

    NodeDescBase1(ident_key<Value>) {}
...
struct NodeDescBase2:
    public NodeDescBase1<Value, RankType, KeyFunc, UnifWeight> {
public:
    using Base = NodeDescBase1<Value, RankType, KeyFunc, UnifWeight>;
    using Base::m_store_weight;
    using typename Base::_node_t;
    ...
    constexpr inline decltype(auto) _key(const _node_t *n) const {
        return this->_value2key(n->m_value);
    }

    constexpr static inline _node_t *&_left(_node_t *n) {
        return n->m_left;
    }

    constexpr static inline _node_t *&_right(_node_t *n) {
        return n->m_right;
    }

    constexpr static inline _node_t *&_parent(_node_t *n) {
        return n->m_parent;
    }

    constexpr static inline INT4 &_hdiff(_node_t *n) {
        return n->m_hdiff;
    }
    ...
    void swap(NodeDescBase2 &other) noexcept {
        ((Base*) this)->swap(other);
        swap(m_less_fn, other.m_less_fn);
    }
};

template<class NodeType, bool StoreWeight>
struct GetNodeWeight {
    static inline decltype(auto) _weight(const NodeType *n) {
        return 1;
    }
};

template<class NodeType>
struct GetNodeWeight<NodeType, true> {
    static inline decltype(auto) _weight(const NodeType *n) {
        return n->m_weight;
    }
};
...
template<class Value, class RankType, class KeyFunc, class Compare,
         bool UnifWeight, class = void>
struct NodeDesc:
    public NodeDescBase2<Value, RankType, KeyFunc, Compare, UnifWeight> {

    using Base = NodeDescBase2<Value, RankType, KeyFunc, Compare, UnifWeight>;
    using Base::m_store_weight;
    using typename Base::_node_t;

    NodeDesc(const Compare &compare, const KeyFunc &key_func):
        Base(compare, key_func) {}
...
template<class Value, class RankType, class KeyFunc, class Compare,
         bool UnifWeight>
struct NodeDesc<Value, RankType, KeyFunc, Compare, UnifWeight,
    std::enable_if_t<std::is_arithmetic_v<RankType>>>:
    public NodeDescBase2<Value, RankType, KeyFunc, Compare, UnifWeight> {
    ...
    static void _fix_agg(_node_t *n) {
        n->m_subtree_weight = GetNodeWeight<_node_t, m_store_weight>::_weight(n);
        if (n->m_left) n->m_subtree_weight += n->m_left->m_subtree_weight;
        if (n->m_right) n->m_subtree_weight += n->m_right->m_subtree_weight;
    }
};

template<class Value, class KeyFunc, class Compare, bool UnifWeight>
struct ValueCompare: public Compare {
    template<class RankType>
    ValueCompare(const NodeDesc<Value, RankType, KeyFunc, Compare, UnifWeight> &desc):
...
template<class Value, class Compare, bool UnifWeight>
struct ValueCompare<Value, ident_key<Value>, Compare, UnifWeight>: public Compare {
    template<class RankType>
    ValueCompare(
        const NodeDesc<Value, RankType, ident_key<Value>, Compare, UnifWeight> &desc):
...
template<
    ...
    class RankType = void, // subtree aggregation enabled if it is arithmetic
    bool UnifWeight = true, // if set to false, insert has an extra RankType
                            // weight arg
...
public:
    ...
    constexpr const static bool m_weight_is_uniform =
        !m_enable_rank_tree_interfaces || UnifWeight;

private:
    typedef NodeDesc<Value, RankType, KeyFunc, Compare, m_weight_is_uniform>
                                                        _node_desc_t;
    typedef avl_t<_node_desc_t, false>                  _avl_t;
    typedef typename _node_desc_t::_node_t              _node_t;
...
public:
    ...
    typedef ValueCompare<Value, KeyFunc, Compare, UnifWeight>
                                                    value_compare; 
...
private:
    ...
    /* 
     * This is used for insertions with stored weights. When rank_type is void,
     * we need to disable them, but compiler will complain if we use const
     * rank_type & as the type of the second parameter (const void &).
     *
     * While we can workaround that without using this rank_type_ by putting
     * std::enable_if over the second parameter type, it's clumsy.
     */
    typedef typename std::conditional<m_enable_rank_tree_interfaces, rank_type, int>::type
                                                    rank_type_;
    ...
    public: 
        ...
        constexpr inline
        typename std::enable_if<b, rank_type>::type weight() const {
            return GetNodeWeight<_node_t, !m_weight_is_uniform>::_weight(m_node);
        }
...
public:
    ...
    avl_container(
        typename std::enable_if<m_weight_is_uniform, InputIt>::type first,
        InputIt last,
    ...
    template<bool b = m_weight_is_uniform>
    avl_container(
        typename std::enable_if<b, std::initializer_list<value_type>>::type init,
        const Compare &comp = Compare(),
        const Allocator &alloc = Allocator(),
        const KeyFunc &key_func = KeyFunc()):
        m_node_alloc(alloc),
    ...
    template<bool b = m_weight_is_uniform>
    typename std::enable_if<b, iterator>::type
    ...
    template<bool b = m_weight_is_uniform>
    typename std::enable_if<!b, iterator>::type
    insert(value_type &&value, rank_type_ weight) {
        _node_t *n = construct_node(m_node_alloc, std::forward<value_type>(value));
        n->m_weight = weight;
        m_avl.insert(n);
        ++m_cnt;
    ...
    insert(
        ...
        _node_t *n = constrct_node(m_node_alloc,
    ...
    /* disabled for non-uniform weight */
    template<class InputIt, bool b = m_weight_is_uniform>
    ...
    template<class C2>
    void merge(avl_container<Value, KeyFunc, C2, RankType, UnifWeight, Allocator> &source) {
...
//
// TODO(zy) I have so many other useful interfaces in avl.h that are not
// incorporated here yet! Do so when you have some free time....
    
    /*
     * Returns my rank (first item is at rank 0, second at rank 1, ...) for
     * UnifWeight, or the prefix-sum weight up to the largest item
     * prior to me.
     */
    template<bool b = m_enable_rank_tree_interfaces>
    ...
    template<bool b = m_enable_rank_tree_interfaces>
    std::enable_if_t<b, rank_type>
    get_rank(const value_type &value) const {
        return get_rank_by_key(m_avl._value2key(value));
    }

    template<class K,
             bool b = m_enable_rank_tree_interfaces &&
                    (m_compare_has_is_transparent ||
                     std::is_same_v<K, key_type>)>
    std::enable_if_t<b, rank_type>
    get_rank(const K &key) const {
        return get_rank_by_key(key);
    }
    
    template<bool b = m_enable_rank_tree_interfaces>
    std::enable_if_t<b, rank_type>
    get_rank_by_key(const key_type &key) const {
        rank_type rank = 0;
        m_avl.template get_sum_left<false>(
            key,
            rank,
    ...
    /*
     * Returns the item at rank n for UnifWeight, or the item that is the
     * smallest item whose prefix sum starting from the smallest item in the
     * entire tree is > n.
     *
     * This can be used for support range sampling for range [lb, ub) if
     * n is generated between [get_rank(lb), get_rank(ub)).
     */
    template<bool b = m_enable_rank_tree_interfaces>
    const_iterator get_nth(
        ...
        _node_t *node = m_avl.get_nth_node(
            ...
            [](rank_type &n, _node_t *node) -> bool {
                if (n < node->m_subtree_weight) return false;
                n -= node->m_subtree_weight;
                return true;
            },
            [](rank_type &n, _node_t* node) -> bool {
                const auto w = GetNodeWeight<_node_t, !m_weight_is_uniform>::_weight(node);
                if (n < w) return false;
                n -= w;
                return true;
            });
...
template<
    ...
    class Allocator>
void swap(
    typename avl_container<Value, KeyFunc, Compare, RankType, UnifWeight, Allocator>::node_type &x,
    typename avl_container<Value, KeyFunc, Compare, RankType, UnifWeight, Allocator>::node_type &y)
    noexcept(noexcept(x.swap(y))) {
    
    x.swap(y);
}

template<class Value, class KeyFunc, class Compare, class RankType, bool UnifWeight, class Allocator>
bool operator==(
    const avl_container<Value, KeyFunc, Compare, RankType, UnifWeight, Allocator> &lhs,
    const avl_container<Value, KeyFunc, Compare, RankType, UnifWeight, Allocator> &rhs) {
...
template<class Value, class KeyFunc, class Compare, class RankType, bool UnifWeight, class Allocator>
void swap(
    avl_container<Value, KeyFunc, Compare, RankType, UnifWeight, Allocator> &lhs,
    avl_container<Value, KeyFunc, Compare, RankType, UnifWeight, Allocator> &rhs)
...
using avl_multiset = avl_container_impl::avl_container<
    Value, avl_container_impl::ident_key<Value>, Compare, void, true, Allocator>;
...
template<
    ...
    class Compare = std::less<Value>,
    class Allocator = std::allocator<Value>>
using avl_weighted_multiset = avl_container_impl::avl_container<
    Value, avl_container_impl::ident_key<Value>, Compare,
    WeightType,
    false,
    Allocator>;
