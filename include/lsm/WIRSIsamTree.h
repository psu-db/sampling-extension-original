...
class WIRS_ISAMTree {
public:
    WIRS_ISAMTree(PagedFile *pfile, const gsl_rng *rng, BloomFilter *tomb_filter, WIRSRun * const* runs, size_t run_cnt, WIRS_ISAMTree * const*trees, size_t tree_cnt) {
        assert(false);
    }
...
private:
    ...
    static PageNum generate_internal_levels(PagedFile *pfile, size_t final_leaf_rec_cnt, char *out_buffer, size_t out_buffer_sz) {
        ...
        PageNum pl_pg_cnt = WIRS_ISAMTree::generate_next_internal_level(pfile, &pl_final_rec_cnt, &pl_first_pg, true, out_buffer, out_buffer_sz, in_buffer, in_buffer_sz);
        ...
        // is 1.
        while ((pl_pg_cnt = WIRS_ISAMTree::generate_next_internal_level(pfile, &pl_final_rec_cnt, &pl_first_pg, false, out_buffer, out_buffer_sz, in_buffer, in_buffer_sz)) != 1) {
            assert(pl_pg_cnt != INVALID_PNUM);
        }
