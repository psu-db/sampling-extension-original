...
public:
    ...
    static MemoryLevel* merge_levels(MemoryLevel* base_level, MemoryLevel* new_level, bool tagging, const gsl_rng* rng) {
        ...
        res->m_structure->m_runs[0] = new CHTRun(runs, 2, res->m_structure->m_bfs[0], tagging);
        return res;
    }
    ...
    void append_mem_table(MemTable* memtable, const gsl_rng* rng) {
        ...
        m_structure->m_runs[m_run_cnt] = new CHTRun(memtable, m_structure->m_bfs[m_run_cnt], m_tagging);
        ++m_run_cnt;
    }
    ...
    void append_merged_runs(MemoryLevel* level, const gsl_rng* rng) {
        ...
        m_structure->m_runs[m_run_cnt] = new CHTRun(level->m_structure->m_runs, level->m_run_cnt, m_structure->m_bfs[m_run_cnt], m_tagging);
        ++m_run_cnt;
    }
    ...
    bool tombstone_check(size_t run_stop, const key_t& key, const key_t& val) {
